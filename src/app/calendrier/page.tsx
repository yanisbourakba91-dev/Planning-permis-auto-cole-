...
function shiftWeek(d: Date, n: number): Date { const r = new Date(d); r.setDate(r.getDate()+n*7); return r; }
function fmt2(n: number) { return String(n).padStart(2,"0"); }
function drivingDate(s: string | null): string {
  if (!s) return "—"; const d = new Date(s); return `${fmt2(d.getDate())}/${fmt2(d.getMonth()+1)}`;
}
...
/* ═══════════════════════════════════════
   DRAGGABLE HOOK
   Uses native addEventListener with { passive: false } so e.preventDefault()
   actually works on iOS/Android (React registers touch listeners as passive).
═══════════════════════════════════════ */
function useDraggableTouch(
  ref: React.RefObject<HTMLElement | null>,
  callbacks: React.RefObject<{
    onDragStart: () => void;
    onTouchMove: (x: number, y: number) => void;
    onTouchEnd: (x: number, y: number, moved: boolean) => void;
  }>
) {
  useEffect(() => {
    ...
    const onStart = (e: TouchEvent) => {
      e.preventDefault(); // stop scroll AND stops click synthesis on iOS
      moved = false;
    ...
    const onMove = (e: TouchEvent) => {
      ...
      moved = true;
      const t = e.touches[0];
      callbacks.current.onTouchMove(t.clientX, t.clientY);
    };
    const onEnd = (e: TouchEvent) => {
      const t = e.changedTouches[0];
      callbacks.current.onTouchEnd(t.clientX, t.clientY, moved);
      moved = false;
    };
...
/* ─────────── QueueCard ─────────── */
function QueueCard({ student, onClickCard, cbs }: {
  student: Student;
  onClickCard: () => void;
  cbs: React.RefObject<{
  ...
  const touchCbs = useRef({
    onDragStart: () => { setDragging(true); cbs.current.onDragStart({ kind: "student", student }); },
    onTouchMove: (x: number, y: number) => { cbs.current.onTouchHover(x, y, ref.current!); },
    onTouchEnd:  (x: number, y: number, moved: boolean) => {
      setDragging(false);
      if (moved) cbs.current.onTouchDrop(x, y, ref.current!);
      else onClickCard(); // tap → open modal
    },
  });
  // keep callbacks fresh without re-running the effect
  useEffect(() => {
    touchCbs.current.onDragStart = () => { setDragging(true); cbs.current.onDragStart({ kind: "student", student }); };
    touchCbs.current.onTouchEnd  = (x, y, moved) => {
      setDragging(false);
      if (moved) cbs.current.onTouchDrop(x, y, ref.current!);
      else onClickCard();
    };
  ...
  useDraggableTouch(ref, touchCbs);
  ...
  return (
    <div
      ...
      draggable
      onDragStart={(e) => {
        e.dataTransfer.effectAllowed = "move"; e.dataTransfer.setData("text/plain","1");
        setDragging(true); cbs.current.onDragStart({ kind: "student", student });
      }}
...
/* ─────────── PlacementChip ─────────── */
function PlacementChip({ placement, onClickChip, cbs }: {
  placement: Placement;
  onClickChip: () => void;
  cbs: React.RefObject<{
    onDragStart: (d: DragData) => void;
  ...
  const touchCbs = useRef({
    onDragStart: () => { setDragging(true); cbs.current.onDragStart({ kind: "placement", placement }); },
    onTouchMove: (x: number, y: number) => { cbs.current.onTouchHover(x, y, ref.current!); },
    onTouchEnd:  (x: number, y: number, moved: boolean) => {
      setDragging(false);
      if (moved) cbs.current.onTouchDrop(x, y, ref.current!);
  ...
  useEffect(() => {
    touchCbs.current.onDragStart = () => { setDragging(true); cbs.current.onDragStart({ kind: "placement", placement }); };
    touchCbs.current.onTouchEnd  = (x, y, moved) => {
      setDragging(false);
      if (moved) cbs.current.onTouchDrop(x, y, ref.current!);
      else onClickChip();
  ...
  return (
    <div
      ...
      onDragStart={(e) => {
        e.dataTransfer.effectAllowed = "move"; e.dataTransfer.setData("text/plain","1");
        setDragging(true); cbs.current.onDragStart({ kind: "placement", placement });
      }}
      onDragEnd={() => setDragging(false)}
...
/* ─────────── TimeSlot ─────────── */
function TimeSlot({ slotId, dateStr, time, children, onClickSlot, isOver, onDragOver, onDragLeave, onDrop }: {
  slotId: string; dateStr: string; time: string; children: React.ReactNode;
  onClickSlot: () => void; isOver: boolean;
...
function EditableCounter({ label, value, sub, color, onSave }: {
  ...
  function start() { if (!onSave) return; setInput(value!==null?String(value):""); setEditing(true); setTimeout(()=>ref.current?.focus(),40); }
...
export default function CalendrierPage() {
  ...
  /* Ghost for touch drag visual feedback */
  const [ghost, setGhost] = useState<{ data: DragData; x: number; y: number }|null>(null);

  function handleDragStart(data: DragData) { dragSrcRef.current = data; }
  ...
  /* Touch: find slot via elementFromPoint (hide self so it's not in the way) */
  ...
  function handleTouchHover(x: number, y: number, self: HTMLElement) {
    if (dragSrcRef.current) setGhost(g => g ? { ...g, x, y } : { data: dragSrcRef.current!, x, y });
  ...
  function handleTouchDrop(x: number, y: number, self: HTMLElement) {
    ...
    const slot = findSlotAt(x, y, self);
    if (slot?.dataset.date && slot?.dataset.time) handleDrop(slot.dataset.date, slot.dataset.time);
    else dragSrcRef.current = null;
  }

  /* Shared callbacks ref passed to all cards — stable object, never recreated */
  const cardCbs = useRef({
    onDragStart:  handleDragStart,
    onTouchHover: handleTouchHover,
    onTouchDrop:  handleTouchDrop,
  });
  /* Keep callbacks fresh each render */
  cardCbs.current.onDragStart  = handleDragStart;
  cardCbs.current.onTouchHover = handleTouchHover;
  cardCbs.current.onTouchDrop  = handleTouchDrop;
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const found = Array.isArray(allMonths)
        ? (allMonths as ExamMonthData[]).find(m=>m.month===activeMonth&&m.year===activeYear)??null : null;
      setMonthData(found);
  ...
  return (
    <AppLayout title="Calendrier" role={session?.user?.role||""} schoolName={session?.user?.schoolName}>
            ...
            <div className="flex flex-1 overflow-y-auto min-h-0">
                ...
                return (
                  <div key={dk} className={cn("flex-1 min-w-0 border-r border-gray-100 dark:border-gray-800 last:border-r-0",isToday&&"bg-blue-50/20")}>
                    {TIME_SLOTS.map(time => {
                      ...
                      return (
                        <TimeSlot key={time} slotId={slotId} dateStr={dk} time={time} onClickSlot={()=>openFromSlot(dk,time)}
                          isOver={dropTarget===slotId} onDragOver={setDropTarget} onDragLeave={()=>setDropTarget(null)} onDrop={handleDrop}>
                          {slotPlacements.map(p=>(
                            <PlacementChip key={p.id} placement={p} cbs={cardCbs}
                              onClickChip={()=>openDetail(p)}/>
                          ))}
                        </TimeSlot>
                      );
                    })}
              ...
              <div className="w-44 flex-shrink-0 border-l-2 border-blue-100 dark:border-blue-900/50 bg-blue-50/20 overflow-y-auto p-2 space-y-1.5">
                {students.length===0 ? (
                  ...
                  <QueueCard key={s.id} student={s} onClickCard={()=>openFromQueue(s)} cbs={cardCbs}/>
                ))}
              </div>
            </div>
          </div>
      ...
      {ghost && (
        <div className="fixed pointer-events-none z-[9999] -translate-x-1/2 -translate-y-1/2 rotate-2 opacity-90"
          ...
          {ghost.data.kind==="student" ? (
            ...
            <div className="bg-gradient-to-br from-blue-500 to-blue-600 text-white text-[10px] font-semibold rounded-lg px-2 py-1.5 shadow-2xl">
              {fullName(ghost.data.placement.student)}
            </div>
          )}
        </div>
      )}
